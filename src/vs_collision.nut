...
class ::Ray_t
{
	...
	function Init( start, end, mins = null, maxs = null ) : (Vector)
	{
		...
		if ( !mins )
		{
			...
			m_Start = Vector( start.x, start.y, start.z );
		}
		else
		{
...
// flTolerance [0..1]
//-----------------------------------------------------------------------------
function VS::IsRayIntersectingSphere( vecRayOrigin, vecRayDelta, vecCenter, flRadius, flTolerance = 0.0 ):(VectorMA)
{
...
function VS::IntersectInfiniteRayWithSphere( vecRayOrigin, vecRayDelta, vecSphereCenter, flRadius, pT ) : (sqrt)
{
	...
	local oo2a = 0.5 / a;
	pT[0] = ( -b - flDiscrim ) * oo2a;
	pT[1] = ( -b + flDiscrim ) * oo2a;
	return true;
}
/*
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
function VS::IntersectRayWithRay( vecStart0, vecDelta0, vecStart1, vecDelta1/* , TS = [0.0,0.0] */ )
{
	//
	// r0 = p0 + v0t
	// r1 = p1 + v1s
...
function VS::IntersectRayWithPlane( org, dir, normal, dist )
{
	...
	return d ? ( dist - org.Dot( normal ) ) / d : 0.0;
}
...
//-----------------------------------------------------------------------------
function VS::ComputeSeparatingPlane( worldToBox1, box2ToWorld, box1Size, box2Size, tolerance, pNormalOut = s_vec )
...
// Returns false if no separating plane exists
//-----------------------------------------------------------------------------
function VS::ComputeSeparatingPlane2( org1, angles1, min1, max1, org2, angles2, min2, max2, tolerance, pNormal = s_vec )
	: (matrix3x4_t)
{
	local worldToBox1 = matrix3x4_t(),
